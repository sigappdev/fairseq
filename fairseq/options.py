...
def sub_args(args):
    "substitute args for Aethar & Philly."
    for k, v in args.__dict__.items():
        if type(v) is not str:
            continue
        if "[#in_dir]" in v:
            print("here")
            assert "in_dir" in args.__dict__, "ERROR>>> in_dir is not in args."
            args.__dict__[k] = v.replace("[#in_dir]", args.in_dir)
        if "[#philly_dir]" in v:
            assert "philly_dir" in args.__dict__, "ERROR>>> philly_dir is not in args."
            args.__dict__[k] = v.replace("[#philly_dir]", args.philly_dir)
        if "[#out_dir]" in v:
            assert "out_dir" in args.__dict__, "ERROR>>> out_dir is not in args."
            args.__dict__[k] = v.replace("[#out_dir]", args.out_dir)
        if "[#log_dir]" in v:
            assert "log_dir" in args.__dict__, "ERROR>>> log_dir is not in args."
            args.__dict__[k] = v.replace("[#log_dir]", args.log_dir)
...
def parse_args_and_arch(parser, input_args=None, parse_known=False, suppress_defaults=False):
    ...
    # sub args if run on Aether & Philly
    sub_args(args)
...
def get_parser(desc, default_task='translation'):
    ...
    # philly, map arg names to aether in/out params
    parser.add_argument('--in_dir', '--inputDirectory', '--datadir', '--dataDir', required=False, type=str)
    parser.add_argument('--philly_dir', '--phillyDirectory', required=False, type=str)
    parser.add_argument('--out_dir', '--outputDirectory', '--modeldir', '--modelDir', required=False, type=str)
    parser.add_argument('--log_dir', '--logDirectory', '--logDir', required=False, type=str)
